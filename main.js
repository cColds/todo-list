...
const taskNavigation = (function () {
	...
	const publishTaskListName = (taskListName) => {
		if (taskListName.includes("inbox")) {
			_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.publish("inbox-task-selected");
		} else if (taskListName.includes("today")) {
			_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.publish("today-task-selected");
		} else if (taskListName.includes("upcoming")) {
			_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.publish("upcoming-task-selected");
		}
	};
...
const taskCard = (function () {
	const render = () => {
		...
		_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.subscribe("today-tasks", (tasksArray) => (0,_task__WEBPACK_IMPORTED_MODULE_1__.todayTask)(tasksArray));
	};

	const updateTaskCounter = () => {
		const taskCounter = document.querySelector(".tasks-counter");
		taskCounter.textContent = `Tasks: ${_task__WEBPACK_IMPORTED_MODULE_1__.tasks[0].length}`;
	};
...
_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.subscribe("today-task-selected", todayTask);

function todayTask() {
	const tasksInToday = [];
	filterTodayTasks(tasksInToday);

	_pubSub__WEBPACK_IMPORTED_MODULE_0__.pubSub.publish("today-tasks", tasksInToday);
}
